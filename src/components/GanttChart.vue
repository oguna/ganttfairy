<template>
    ...
    <title
      ...
      lang="en"
    >{{title}}</title>
    <rect v-for="(rect,index) in weekendRects" :key="'weekend'+index" :x="rect.x" :y="rect.y" :width="rect.w" :height="rect.h" class="weekend"></rect>
    <template v-for="(rect,index) in boxes">
        <rect @mousedown="taskStartMousedown(index,$event)" class="grip" :key="'leftgrip'+index" :x="rect.x-8" :y="rect.y" width="8px" :height="rect.h" />
        <rect @mousedown="taskMousedown(index,$event)" :key="index"
        :x="rect.x" :y="rect.y" :width="rect.w" :height="rect.h" class="box" :class="{selected:(selectedRect === index)}" />
        <rect @mousedown="taskEndMousedown(index,$event)" class="grip" :key="'rightgrip'+index" :x="rect.x+rect.w" :y="rect.y" width="8px" :height="rect.h" />
    </template>
    <g font-family="sans-serif" font-size="12" dominant-baseline="middle">
        <text v-for="(task,index) in tasks" :key="index"
        :x="taskLabels[index].x + 12" :y="taskLabels[index].y" fill="blue">
            {{task.title}}
        </text>
    </g>
    </svg>
...
export default class GanttChart extends Vue {
    ...
    public get boxes(): Rect[] {
        ...
        for (let i = 0; i < this.tasks.length; i++) {
            ...
            let w = (differenceInCalendarDays(task.end, task.start) + 1) * this.dateWidth;
            if (w < 0) {
                w = -w;
            }
            if (x < 0) {
